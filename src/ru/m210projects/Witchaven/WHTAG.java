...
public class WHTAG {
	...
	public static Delayitem delayitem[] = new Delayitem[MAXSECTORS];

	static short ironbarsector[] = new short[16];
	static short ironbarscnt;
	static int[] ironbarsgoal1 = new int[16], ironbarsgoal2 = new int[16];
	static short[] ironbarsdone = new short[16], ironbarsanim = new short[16];
	static int[] ironbarsgoal = new int[16];
	...
	public static short ypanningwalllist[] = new short[128], ypanningwallcnt;
	...
	public static void operatesprite(PLAYER plr, short s) {
		...
		case STONEGONZOBSW:
			sprite[s].lotag *= 120;
			engine.changespritestat((short) s, STONETOFLESH);
			break;
		case GONZOHMJUMP:
		case GONZOSHJUMP:
		...
		case STAINSCENE:
			...
			case 2:
				playsound_loc(S_GLASSBREAK1 + (engine.rand() % 3), sprite[s].x, sprite[s].y);
				for (int j = 0; j < 20; j++) {
					shards(s, 2);
				}
				engine.deletesprite((short) s);
				break;
			}
			break;
		}
	...
	public static void operatesector(PLAYER plr, int s) {
		...
		case DOORUPTAG: // a door that opens up
			...
			if (i >= 0) {
				goalz = sector[engine.nextsectorneighborz(s, sector[s].floorz, -1, -1)].ceilingz;
		...
		case DOORDOWNTAG: // a door that opens down
			...
			if (i >= 0) {
				goalz = sector[engine.nextsectorneighborz(s, sector[s].ceilingz, 1, 1)].floorz;
				if (gAnimationData[i].goal == goalz) {
		...
		case DOORSPLITHOR:
			...
			j = getanimationgoal(sector[s], 2);
			if (i >= 0) {
				botz = sector[engine.nextsectorneighborz(s, sector[s].ceilingz, 1, 1)].floorz;
			...
			if (j >= 0) {
				topz = sector[engine.nextsectorneighborz(s, sector[s].floorz, -1, -1)].ceilingz;
		...
		if (datag >= 1800 && datag <= 1899) {
			...
			if (i >= 0) {
				...
				if (gAnimationData[i].goal == daz)
					gAnimationData[i].goal = sector[engine.nextsectorneighborz(s, sector[s].ceilingz - (1024 * 16), -1,
							-1)].floorz;
				else
					gAnimationData[i].goal = daz;
			} else {
				if (sector[s].floorz == sector[s].ceilingz + (1024 * 16))
					daz = sector[engine.nextsectorneighborz(s, sector[s].ceilingz - (1024 * 16), -1, -1)].floorz;
				else {
					daz = sector[s].ceilingz + (1024 * 16);
				}
		...
		if (datag >= 2000 && datag <= 2999) {
			System.err.println("WHTAG.java: 683 check this place keychecking"); //XXX
	...
	public static void animatetags(int nPlayer) {
		...
		for (i = 0; i < dragsectorcnt; i++) {
			...
			for (j = startwall; j <= endwall; j++)
				engine.dragpoint((short) j, wall[j].x + dragxdir[i], wall[j].y + dragydir[i]);
			...
			if (plr.sector == dasector) {
				...
				engine.setsprite(plr.spritenum, plr.x, plr.y, plr.z + (plr.height));
			...
			if (swingdoor[i].anginc != 0) {
				...
				if (swingdoor[i].anginc != 0) {
					if (plr.sector == swingdoor[i].sector) {
						good = 1;
						for (k = 1; k <= 3; k++) {
							if (engine.clipinsidebox(plr.x, plr.y, (short) swingdoor[i].wall[k], 512) != 0) {
								good = 0;
								break;
							}
						}
