...
import static ru.m210projects.Witchaven.AI.Ai.SKULLYTYPE;
...
import static ru.m210projects.Witchaven.WH1Names.SKULLY;
import static ru.m210projects.Witchaven.WH1Names.SKULLYATTACK;
import static ru.m210projects.Witchaven.WH1Names.SKULLYCHAR;
import static ru.m210projects.Witchaven.WH1Names.SKULLYDEAD;
...
import static ru.m210projects.Witchaven.WHSND.S_SKULLWITCH1;
import static ru.m210projects.Witchaven.WHSND.playsound_loc;

import ru.m210projects.Build.Types.SPRITE;
import ru.m210projects.Witchaven.Types.PLAYER;

public class AISkully {

	public static void create() {
		enemy[SKULLYTYPE] = new Enemy();
		enemy[SKULLYTYPE].info = new EnemyInfo(32, 32, 2048, 120, 0, 64, false, 300, 0);
		enemy[SKULLYTYPE].chase = new AIState() {
			@Override
		...
		enemy[SKULLYTYPE].resurect = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
				SPRITE spr = sprite[i];
		...
		enemy[SKULLYTYPE].search = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
		...
		enemy[SKULLYTYPE].nuked = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
				SPRITE spr = sprite[i];
		...
		enemy[SKULLYTYPE].pain = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
				SPRITE spr = sprite[i];
				spr.lotag -= TICSPERFRAME;
		...
		enemy[SKULLYTYPE].face = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
				SPRITE spr = sprite[i];
		...
		enemy[SKULLYTYPE].attack = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
				SPRITE spr = sprite[i];
		...
		enemy[SKULLYTYPE].flee = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
				SPRITE spr = sprite[i];
				spr.lotag -= TICSPERFRAME;
		...
		enemy[SKULLYTYPE].cast = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
				SPRITE spr = sprite[i];
				...
				if (spr.picnum == SKULLYATTACK + 2) {
					...
					playsound_loc(S_SKULLWITCH1 + engine.krand() % 3, sprite[i].x, sprite[i].y);
		...
		enemy[SKULLYTYPE].die = new AIState() {
			@Override
			public void process(PLAYER plr, short i) {
				SPRITE spr = sprite[i];
				spr.lotag -= TICSPERFRAME;
