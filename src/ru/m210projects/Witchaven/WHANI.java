	...
	public static void animateobjs(PLAYER plr) {
		...
		if (game.WH2) {
			for (i = headspritestat[SHARDOFGLASS]; i >= 0; i = nextsprite) {
				...
				case 3:
					spr.zvel -= TICSPERFRAME << 5;
					movestat = (short) movesprite(i, ((sintable[(spr.ang + 512) & 2047]) * TICSPERFRAME) << 2,
			...
			for (i = headspritestat[SPARKSUP]; i >= 0; i = nextsprite) {
				...
				if (spr.lotag < 0) {
					...
					movesprite(i, ((sintable[(spr.ang + 512) & 2047]) * TICSPERFRAME) << 3,
					...
					if (osectnum != spr.sectnum) {
						...
						spr.y = sparksy;
						spr.z = sparksz;
						spr.ang = (short) ((engine.krand() % 2047) & 2047);
						spr.zvel = 0;
			...
			for (i = headspritestat[EVILSPIRIT]; i >= 0; i = nextsprite) {
				...
				if (spr.picnum >= (GONZOBSHDEAD - 8)) {
					...
					j = engine.insertsprite(spr.sectnum, FACE);
			...
			for (i = headspritestat[TORCHFRONT]; i >= 0; i = nextsprite) {
				...
				engine.setsprite(i, plr.x, plr.y, plr.z);
				osectnum = spr.sectnum;
		...
		for (i = headspritestat[FLOCKSPAWN]; i >= 0; i = nextsprite) {
			...
			if (sprite[i].lotag < 0) {
				sprite[i].extra--;
				sprite[i].lotag = (short) (engine.krand() & 48 + 24);
				bats(plr, i);
				if (sprite[i].extra == 0)
					engine.changespritestat(i, (short) 0);
			}
		...
		for (i = headspritestat[FLOCK]; i >= 0; i = nextsprite) {
			...
			case 0: // going out of the cave
				if (sprite[i].lotag < 0) {
					...
					movestat = (short) movesprite(i,
							((sintable[(sprite[i].ang + 512) & 2047]) * TICSPERFRAME) << 3,
							((sintable[sprite[i].ang & 2047]) * TICSPERFRAME) << 3, 0, 4 << 8, 4 << 8, 0);
			...
			case 1: // flying in circles
				if (sprite[i].lotag < 0) {
					sprite[i].extra = 2;
					sprite[i].lotag = 512;
					sprite[i].ang = (short) (((engine.getangle(sprite[sprite[i].hitag].x - sprite[i].x,
...
//			startredflash(j);
		...
		for (i = headspritestat[MASPLASH]; i >= 0; i = nextsprite) {
			...
			game.pInt.setsprinterpolate(i, sprite[i]);
			sprite[i].lotag -= TICSPERFRAME;
			sprite[i].z = sector[sprite[i].sectnum].floorz + (engine.getTile(sprite[i].picnum).getHeight() << 8);
			engine.setsprite(i, sprite[i].x, sprite[i].y, sprite[i].z);
		...
		for (i = headspritestat[FALL]; i >= 0; i = nextsprite) {
			...
			engine.getzrange(sprite[i].x, sprite[i].y, sprite[i].z - 1, sprite[i].sectnum, (sprite[i].clipdist) << 2,
					CLIPMASK0);
		...
		for (i = headspritestat[MISSILE]; i >= 0; i = nextsprite) {
			...
			case FIREBALL:
			case PLASMA:
				if ((sprite[i].picnum == WH1THROWPIKE && game.WH2) || (sprite[i].picnum == WH2THROWPIKE && !game.WH2))
					break;
				...
				if (sprite[i].picnum == MONSTERBALL && engine.krand() % 100 > 90) {
...
//					}
			...
			if (hitobject != 0 || sprite[i].lotag < 0) {
				...
				case WH1THROWPIKE:
				case WH2THROWPIKE:
					if ((sprite[i].picnum == WH1THROWPIKE && game.WH2)
							|| (sprite[i].picnum == WH2THROWPIKE && !game.WH2))
						break;
...
//				}
		...
		for (i = headspritestat[CHUNKOMEAT]; i >= 0; i = nextsprite) {
			...
			movestat = (short) movesprite(i, xvel, yvel, daz, 4 << 8, 4 << 8, 1);
		...
		for (i = headspritestat[DEVILFIRE]; i >= 0; i = nextsprite) {
			...
			if (plr.invisibletime < 0) {
				sprite[i].lotag -= TICSPERFRAME;
				if (sprite[i].lotag < 0) {
					sprite[i].lotag = (short) (engine.krand() & 120 + 360);
...
//			engine.setsprite(i, sprite[i].x, sprite[i].y, sprite[i].z);
		...
		if (!game.WH2) {
			for (i = headspritestat[EXPLO]; i >= 0; i = nextsprite) {
				...
				if (engine.krand() % 100 > 90) {
					j = engine.insertsprite(sprite[i].sectnum, SMOKE);
		...
		for (i = headspritestat[FX]; i >= 0; i = nextsprite) {
			...
			sprite[i].picnum == BULLET || sprite[i].picnum == EXPLOSION || sprite[i].picnum == FIREBALL
				...
				dax = (((sprite[i].xvel) * TICSPERFRAME) >> 3);
				day = (((sprite[i].yvel) * TICSPERFRAME) >> 3);
				daz = ((sprite[i].zvel) * TICSPERFRAME);
				movestat = (short) movesprite(i, dax, day, daz, 4 << 8, 4 << 8, 1);
				engine.setsprite(i, sprite[i].x, sprite[i].y, sprite[i].z);
			}
