...
import static ru.m210projects.Blood.Actor.kVectorGhost;
...
import static ru.m210projects.Blood.DB.kMissileEctoSkull;
...
public class AIGHOST {
	...
	public static void Init()
	{
		...
		ghostBite = new AISTATE(Type.other,6, new CALLPROC() {
			public void run(int nXSprite) {
				BiteCallback(nXSprite);
			}
		}, 120, false, false, false, ghostChase );
		ghostThrow	= new AISTATE(Type.other,6, new CALLPROC() {
			public void run(int nXSprite) {
				ThrowCallback(nXSprite);
			}
		}, 120, false, true, false, ghostChase ) {
			public void move(SPRITE sprite, XSPRITE xsprite) {
	...
	private static void BiteCallback( int nXIndex )
	{
		...
		int dz = dudeInfo[pSprite.lotag - kDudeBase].eyeHeight * pSprite.yrepeat << 2;
				...
				if ( dist < pDudeInfo.seeDist && klabs(losAngle) <= pDudeInfo.periphery )
				{
					...
					if(dist > 4096 && dist < 8192 && klabs(losAngle) < kAngle15)
					{
						...
						if(nInfo == SS_WALL || nInfo == SS_MASKED)
							return;
						if(nInfo != SS_SPRITE || (nInfo == SS_SPRITE 
								&& pSprite.lotag != sprite[pHitInfo.hitsprite].lotag 
								&& sprite[pHitInfo.hitsprite].lotag != kDudePhantasm))
