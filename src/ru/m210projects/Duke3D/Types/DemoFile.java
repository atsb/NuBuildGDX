...
import static ru.m210projects.Duke3D.Globals.BYTEVERSION;
import static ru.m210projects.Duke3D.Globals.BYTEVERSION15;
import static ru.m210projects.Duke3D.Globals.GDXBYTEVERSION;
import static ru.m210projects.Duke3D.Globals.JFBYTEVERSION;
import static ru.m210projects.Duke3D.Globals.RECSYNCBUFSIZ;
...
import static ru.m210projects.Duke3D.Globals.sync;
...
public class DemoFile {
	...
	public DemoFile(String filename) throws Exception {
		...
		auto_run = recfilep.readInt();
		int MAX_PATH = 128;
		if(version >= JFBYTEVERSION)
			MAX_PATH = 260;

		recfilep.read(tempbuf, 0, MAX_PATH);
		String name = FileUtils.getCorrectPath(new String(tempbuf, 0, MAX_PATH).trim());
	...
	public DemoFile(int nVersion)
	{
		...
		res.writeInt(ud.auto_run);
		int MAX_PATH = 128;
		if (nVersion >= JFBYTEVERSION)
			MAX_PATH = 260;

		if (boardfilename != null)
			res.writeBytes(boardfilename.toCharArray(), MAX_PATH);
		else res.writeBytes(new byte[MAX_PATH], MAX_PATH);
