...
public class VOC extends SoundData {
		...
		while (true) {
			...
			switch (blocktype) {
				case 1: /* sound data begin block */
					rate = 1000000 / (256 - (data[tptr++] & 0xFF));
					if (data[tptr++] != 0) {
				...
				case 8: /* sound attribute extension block */
					int samplerate = LittleEndian.getShort(data, tptr); tptr += 2;
					rate = (256000000/(65536-(samplerate & 0xFFFF)));
					bits = 8;
				...
				case 9: /* sound data format 3 */
					rate = LittleEndian.getInt(data, tptr); tptr += 4;
